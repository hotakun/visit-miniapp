...
# 文件路径
input_file = r"D:\WFR\visit-miniapp\TMP\fenqu\西湖区.xlsx"
output_dir = r"D:\WFR\visit-miniapp\TMP\fenqu\xihu"
...
try:
    ...
    if 'regionName' not in df.columns:
        ...
        print(f"找不到'regionName'列，可能的列有: {possible_cols}")
        if possible_cols:
            ...
            for i, col in enumerate(df.columns):
                print(f"  {i+1:2}. {col}")
            raise KeyError("无法找到regionName列")
    else:
    ...
    print(f"\n共有 {len(region_counts)} 个不同的 regionName 值")
    ...
    if total_exported < len(df):
        print(f"注意: 有 {len(df) - total_exported} 条记录未被分配（regionName列为空或处理失败）")
    
    # 生成统计摘要文件
    stats_file = os.path.join(output_dir, "区域统计信息.txt")
    with open(stats_file, 'w', encoding='utf-8') as f:
        f.write(f"西湖区按 regionName 分组统计\n")
        f.write(f"="*50 + "\n")
